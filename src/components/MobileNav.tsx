  ...
  return (
    <div className="flex lg:hidden">
      <button onClick={toggle} className="flex flex-col w-8 h-6 justify-center items-center fixed top-0 right-0 m-4">
        <span
          className={`w-full h-0.5 rounded-sm bg-slate-900 left-0 top-0 absolute
          transition-all duration-300 ease-out
          ${isOpen ? 'top-3 rotate-45' : 'rotate-0'}`}
        >
        </span>
        <span className={`w-full h-0.5 rounded-sm bg-slate-900
        transition-all duration-300 ease-out
        ${isOpen ? 'opacity-0' : 'w-full'}`}>
        </span>
        <span className={`w-full h-0.5 rounded-sm bg-slate-900 bottom-0 absolute
          transition-all duration-300 ease-out
