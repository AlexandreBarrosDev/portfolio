...
export default function RootLayout({
  ...
  return (
    <html lang="pt-BR" className='bg-zinc-800'>
      <body className={`${roboto.className} antialiased bg-gradient-to-br from-zinc-800 text-zinc-200`}>
        <header
          className={`${raleway.className} fixed flex lg:flex-col lg:justify-between items-center top-0 left-0
          w-full h-14 lg:w-48 lg:h-full p-5 bg-zinc-200`}
        >
          <nav className='flex lg:flex-col lg:justify-between items-center gap-2 lg:gap-8 w-full'>
            <Link href="/">
              <Image src="/alx-logo.svg" alt="Logo Alexandre Barros" width={30} height={30}/>
            </Link>
            <MobileNav />
            <Navbar />
            <h1
              className={`${raleway.className} antialiased
              text-zinc-800 lg:mt-60 lg:block lg:-rotate-90 lg:text-5xl
              lg:text-nowrap`}
            >
              ALEXANDRE BARROS
            </h1>
        ...
        <footer className="flex justify-center items-center fixed bottom-0 w-full lg:ml-28 p-2 text-xs text-zinc-200
